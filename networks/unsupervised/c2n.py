...
class C2N_D(nn.Module):
    def __init__(self, n_ch_in):
        ...
        self.tail = nn.Conv2d(self.n_ch_unit, n_ch_in, 3,
                              padding=1, bias=True)
...
class C2N_G(nn.Module):
    ...
    def forward(self, x, r_vector=None):
        ...
        normal_scale = F.relu(feat_CL[:, self.n_ch_unit:, :, :]) + ddd
...
if __name__ == '__main__':
    ...
    c2nd = C2N_D(n_ch_in = 3).cuda()
    print(c2nd(x).shape)
    c2ng = C2N_G(n_ch_in=3, n_ch_out=3, n_r=32).cuda()
    print(c2ng(x).shape)

    x = torch.randn(2, 1, 128, 128).cuda()
    c2nd = C2N_D(n_ch_in = 1).cuda()
    print(c2nd(x).shape)
    c2ng = C2N_G(n_ch_in=1, n_ch_out=1, n_r=32).cuda()
    print(c2ng(x).shape)
