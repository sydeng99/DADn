...
from blind2unblind import UNet
import utils_blind2unblind as util
...
parser.add_argument('--data_dir', type=str,default='dataset/trainset/noisy')
parser.add_argument('--val_dirs', type=str, default='dataset/validset')
parser.add_argument('--save_model_path', type=str,default='experiments/b2u/')
parser.add_argument('--log_name', type=str,default='b2u_unet')
...
def validation(dataset_dir):
    noisy_fns = glob.glob(os.path.join(dataset_dir+'/noisy/', "*"))
    noisy_fns.sort()
    noisy_images = []
    for fn in noisy_fns:
        im = Image.open(fn)
    ...
    clean_fns = glob.glob(os.path.join(dataset_dir+'/clean/', "*"))
    clean_fns.sort()
    clean_images = []
    for fn in clean_fns:
        im = Image.open(fn)
...
# Validation Set
Validation_dir = os.path.join(opt.val_dirs)
...
# }
valid_dict = {
    "valid": validation(Validation_dir)
}
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        for valid_name, valid_images in valid_dict.items():
            ...
            for i in range(repeat_times):
                for idx, ims in enumerate(valid_images):
                    noisy_im, origin255 = ims[0], ims[1]
                    # origin255 = im.copy()
                    ...
                    noisy_im = np.pad(noisy_im,[[0, val_size - H], [0, val_size - W]],'reflect')
                    ...
                    if i == 0 and epoch == opt.n_snapshot:
                        ...
                        io.imsave(save_path, origin255.astype('uint8'))
                        # Image.fromarray(origin255).save(save_path)
                        # Image.fromarray(origin255).convert('RGB').save(save_path)
                        save_path = os.path.join(
                            save_dir,
                            "{}_{:03d}-{:03d}_noisy.png".format(
                                valid_name, idx, epoch))
                        io.imsave(save_path, noisy255.astype('uint8'))
                        # Image.fromarray(noisy255).save(save_path)
                    ...
                    if i == 0:
                        save_path = os.path.join(
                            ...
                            "{}_{:03d}-{:03d}_dn.png".format(
                                valid_name, idx, epoch))
                        # Image.fromarray(pred255_dn).save(save_path)
                        io.imsave(save_path, pred255_dn.astype('uint8'))

                        save_path = os.path.join(
                            save_dir,
                            "{}_{:03d}-{:03d}_exp.png".format(
                                valid_name, idx, epoch))
                        # Image.fromarray(pred255_exp).save(save_path)
                        io.imsave(save_path, pred255_exp.astype('uint8'))

                        save_path = os.path.join(
                            save_dir,
                            "{}_{:03d}-{:03d}_mid.png".format(
                                valid_name, idx, epoch))
                        # Image.fromarray(pred255_mid).save(save_path)
                        io.imsave(save_path, pred255_mid.astype('uint8'))
