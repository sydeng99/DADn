...
if __name__ =='__main__':
    a = torch.randn(2,1,128,128).cuda()
    net = UNet(nch_in=1, nch_out=1).cuda()
    out = net(a)
    print(out.shape)   # torch.Size([2, 1, 128, 128])

    dnet = Discriminator(nch_in=1).cuda()
    dout = dnet(a)
    print(dout.shape)  # torch.Size([2, 1, 7, 7])

    resnet = ResNet(nch_in=1, nch_out=1).cuda()
    rout = resnet(a)
    print(rout.shape)  # torch.Size([2, 1, 128, 128])